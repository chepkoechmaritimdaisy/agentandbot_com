defmodule GovernanceCore.AXAudit do
      ...
      case result do
        {:ok, %{status: 200, body: body}} ->
          case Jason.decode(body) do
            {:ok, _json} -> {:ok, url}
            {:error, reason} -> {:error, "MCP Endpoint #{url} invalid JSON: #{inspect(reason)}"}
          end
        {:ok, %{status: status}} ->
  ...
  defp handle_failures(failures) do
    ...
    File.write!(fix_path, "Automated Fix for AX Audit Failures: \n" <> inspect(failures))

    try do
      # Add the file
      case System.cmd("git", ["add", fix_path]) do
        {_, 0} ->
          # Commit the change
              ...
              case System.cmd("gh", ["pr", "list", "--search", "🤖 [AX Audit] Automated Fix"]) do
                {output, 0} ->
                  if String.trim(output) == "" do
                    System.cmd("gh", ["pr", "create", "--title", "🤖 [AX Audit] Automated Fix", "--body", "Automated fix generated by AX Audit."])
                  end
                _ -> :ok
              end
            _ -> :ok
      ...
      e in ErlangError -> Logger.error("Failed to create automated PR: #{inspect(e)}")
    end
  end
